...
export function CourseCard({ title, instructor, progress, imageSeed, totalChapters, completedChapters, timeRemaining, delay = 0 }: CourseCardProps) {
  return (
    <motion.div
      ...
      className="panel group max-w-sm rounded-[24px] overflow-hidden flex flex-col cursor-pointer border border-line hover:border-primary/25 hover:shadow-[0_10px_40px_-10px_rgba(59,130,246,0.3)] transition-all ease-out duration-300 relative"
    >
      <div className="relative h-48 w-full overflow-hidden">
        <div className="absolute inset-0 bg-surface group-hover:bg-transparent transition-colors z-10" />
        <div
          className="h-full w-full transition-transform duration-700 group-hover:scale-105"
          style={{ background: gradientFor(imageSeed || title) }}
        />
        <div className="absolute top-4 left-4 z-20 panel px-3 py-1.5 rounded-full text-xs font-medium border-line-strong">
          Core Module
        </div>
      ...
      <div className="p-6 flex flex-col flex-1">
        <div className="mb-4">
          <h3 className="font-outfit text-xl font-semibold mb-1 line-clamp-2 leading-tight group-hover:text-primary transition-colors">
            {title}
          </h3>
          <p className="text-sm text-ink-muted">{instructor}</p>
        </div>

        <div className="mt-auto space-y-4">
          <div className="flex items-center justify-between text-xs text-ink-muted">
            <span className="flex items-center gap-1.5"><BookOpen size={14} className="text-primary" /> {completedChapters}/{totalChapters} Chapters</span>
