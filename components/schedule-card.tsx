...
export function ScheduleCard({ title, time, duration, type, location, instructorUrl, color = 'blue', delay = 0 }: ScheduleCardProps) {
  ...
  return (
    <motion.div
      ...
      className={`panel p-4 rounded-2xl border-l-[3px] relative overflow-hidden group hover:bg-ink/[0.06] transition-colors cursor-pointer
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="text-sm font-semibold tracking-wide text-ink">{time}</span>
            <span className="w-1 h-1 rounded-full bg-slate-600"></span>
            <span className="text-xs text-ink-muted font-medium">{duration}</span>
          </div>
          <h4 className="font-outfit font-medium text-ink group-hover:text-ink transition-colors">{title}</h4>
        </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-line">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-ink-muted">Dr. Sarah Jenkins</span>
        </div>
        
        {location && (
          <div className="text-xs text-ink-subtle bg-surface px-2 py-1 rounded-md border border-line">
            {location}
          </div>
        )}
      </div>
