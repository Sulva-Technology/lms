...
export function RoleBadge({ className, role, ...props }: RoleBadgeProps) {
  const getRoleConfig = () => {
    switch (role) {
      case "admin":
        return { icon: Shield, text: "Admin", classes: "bg-fuchsia-500/10 text-fuchsia-400 border-fuchsia-500/20" };
      case "lecturer":
        return { icon: BookOpen, text: "Lecturer", classes: "bg-blue-500/10 text-blue-400 border-blue-500/20" };
      case "student":
        return { icon: null, text: "Student", classes: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" };
      default:
