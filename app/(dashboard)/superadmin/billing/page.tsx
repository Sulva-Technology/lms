...
export default async function SuperadminBillingPage() {
  ...
  return (
    <GenericList title="Billing" description="Assign plans and track subscription status by university." icon={CreditCard}>
      <form action={assignPlan} className="grid gap-4 rounded-[28px] border border-line bg-surface p-6 shadow-2xl backdrop-blur-2xl lg:grid-cols-4">
        ...
        <button className="rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-contrast hover:bg-primary-hover">Update subscription</button>
      </form>
      <DataTable
        data={universities}
        keyExtractor={(item: any) => item.id}
        columns={[
          { key: "university", header: "University", cell: (item: any) => <span className="font-medium text-ink">{item.name}</span> },
