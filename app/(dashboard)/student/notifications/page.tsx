...
export default async function StudentNotificationsPage() {
  ...
  return (
    <GenericList title="Notifications" description="Actionable alerts across courses, registration, and grading." icon={BellRing}>
      <form action={markAll}>
        <button className="w-fit rounded-xl border border-line bg-status-soft px-4 py-2 text-sm font-medium text-ink hover:bg-ink/[0.06]">Mark all as read</button>
      </form>
      {notifications.length === 0 ? (
        ...
        <DataTable
          ...
          columns={[
            { key: "title", header: "Notification", cell: (item: any) => <span className={item.is_read ? "text-ink-muted" : "font-medium text-ink"}>{item.title}</span> },
            ...
            { key: "action", header: "Action", cell: (item: any) => (
              <div className="flex items-center gap-3">
                {item.link_url ? <Link className="text-primary hover:text-primary" href={item.link_url}>Open</Link> : null}
